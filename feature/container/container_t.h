#ifndef _CONTAINER_T_H_
#define _CONTAINER_T_H_
...
private:
	...
	size_type get_adjusted_count(void) const
	{
		if (step_ > 0 && begin_ >= end_)
			throw std::logic_error("End value small");
		else
			if (step_ < 0 && begin_ <= end_)
				throw std::logic_error("step negative, end value greater");
